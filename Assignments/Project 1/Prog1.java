...
/* Prog1 class: Contains the main method and methods relating*/
...
public class Prog1
{
   ...
   static String courseCode; //the course code in the format "EE-###" (i.e. CS-102)
   ...
   public static void main(String[] args)
   {
      ...
      while (file.hasNext()) //loops as long as the file contains more input
         {  
            ...
            myDatabase.add(index, course); //adds the new course object to the course array contained within database
...
/**************************************************************/
   ...
   public static void userInterface(Scanner scn, Database database)
   {
     while (true) //loops infinitely
     {
      ...
      System.out.println("1 --> Search for a course number");  //input
      System.out.println("2 --> Search course titles");        //options
      System.out.println("3 --> Print all records");
      System.out.println("4 --> Compute GPAs");
      ...
      if (input == 1) 
      {
         ...
         String searchTerm = scn.nextLine(); //assign to user input
         database.searchByNum(searchTerm); //calls the database search method 
      }
      if (input == 2)
      {
         System.out.println("Please enter the title of the course you would like to search for: ");   //prompt for search term
         String searchTitle = scn.nextLine(); //assign to user input
         database.searchByTitle(searchTitle); //calls the database search by title method 
      }
      if (input == 3)
      {
