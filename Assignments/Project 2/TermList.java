...
/**************************************************************/
   ...
   public void printAll()
   {
      ...
      {
         CourseList list = current.getCourseList(); //CourseList of current TermNode
         list.printAll(); //calls CourseList's print method
...
/**************************************************************/
   ...
   public void searchByNum(String courseNum)
   {
      ...
      {
         CourseList list = current.getCourseList(); //CourseList of current TermNode
         list.searchByNum(courseNum); //calls a CourseList method to search
...
/**************************************************************/  
   ...
   public void searchByTitle(String courseTitle)
   {
      ...
      {
         CourseList list = current.getCourseList(); //CourseList of current TermNode
         list.searchByTitle(courseTitle); //calls a CourseList method to search
         current = current.getNextTerm(); //gets next TermNode
      }
   }
...
/**************************************************************/
   ...
   public void addCourse(Course course)
   {
      ...
      int term = Integer.parseInt(course.getCourseYear()); //term number of parameter Course
      ...
      {                                             //and doesn't contain a match
         ...
         {
           CourseList tempList = current.getCourseList(); //current TermNode's CourseList
           tempList.addToCourseList(course); //call to CourseList add
           match = true; //match is true, breaking the loop
         }
      }   
   }
...
/**************************************************************/
   ...
   public void removeCourse(String courseCode, int term)
   {
      ...
      {                                             //and doesn't contain a match
         ...
         {
            CourseList temp = current.getCourseList(); //temporary course object
            temp.removeFromCourseList(courseCode); //call to CourseList remove method
            match = true; //match is true, breaking loop
         }
         else //if term doesn't exist
         {
...
/**************************************************************/
   ...
   public double getGPA()
   {
      ...
      {
         CourseList list = current.getCourseList(); //CourseList of current TermNode
         totalCredits += list.getTotalCredits(); //calls CourseList's totalCredit method
         sum += list.getSum(); //adds the current course's sum to the total sum
         current = current.getNextTerm(); //gets next TermNode
      }
