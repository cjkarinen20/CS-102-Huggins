...
/**************************************************************/
   ...
   public static void userInterface(Scanner scn, Database database)
   {
     while (true) //loops infinitely
     {
      ...
      System.out.println("6 --> Write database to disc");
      System.out.println("7 --> Re-initialize database");
      System.out.println("8 --> Print all records");
      System.out.println("9 --> Compute GPAs");
      ...
      if (input == 5)
      {
         ...
         int academicTerm = Integer.parseInt(scn.nextLine()); //input for the term
         System.out.println("Please enter the course code: ");//and course code
         String searchCode = scn.nextLine(); //input for the course code
         System.out.println("Edit this course? (Y or N): "); //prompt for confirmation
         
         if (scn.nextLine().equals("Y") || scn.nextLine().equals("y")) //if answer is yes
         {
            if (database.searchByNum(searchCode) == true) //if the course exists
            {
               database.removeCourse(academicTerm, searchCode); //call to database remove method
               System.out.println("Please enter the course year (YYYYTT): ");    //Prompt user
               courseYear = scn.nextLine();                                      //for required 
      ...
      if (input == 6)
      {
         System.out.println("Please the name of the file to write to: ");
         String fileName = scn.nextLine(); //the user specified file
      ...
      if (input == 7)
      {
         System.out.println("Please specify a new input file: ");
         String fileName = scn.nextLine();  //the user specified file
         File newInput = new File(fileName); //the new input file
         Scanner newFile = null; //default value
         try 
         {
            ...
            {  
               ...
               database.addCourse(course); //calls database method to add the course
            }
         }
         catch (FileNotFoundException exc) //if file does not exist
         {
           System.out.println("File could not found, database is unchanged...");
         }
      }
